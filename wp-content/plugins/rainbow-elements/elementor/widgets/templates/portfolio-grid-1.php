...
$filter_class = ($project_filter == 'yes') ? 'rn-filterable-portfolios' : '';
$filter_item_class = ($project_filter == 'yes') ? 'rn-filterable-portfolio-item' : '';

?>
<div class="rn-portfolio-area rb-projects portfolio-style-three" data-settings='<?php echo json_encode($data); ?>'
     data-paged='1' id="inbio-portfolio-<?php echo esc_attr(uniqid()) ?>">
<?php 
    if($project_filter == 'yes') { 
            $category_list = '';
    ...
    ?>
            <?php if ($category_list_value && !is_wp_error($category_list_value)) : ?>
                ...
                <div class="row mt--30">
                    <div class="col-lg-12">
                        <div class="rainbow-portfolio-filter filter-button-default messonry-button text-center">
                            <div class="messonry-button pv-tab-button justify-content-center" data-isotope-id="#<?php echo $elemid; ?>">

                                <?php if (!empty($settings['project_filter_all_button_label'])) { ?>
                                    <button data-filter="*" class="is-checked rn-btn"><span class="filter-text"><?php echo esc_html($settings['project_filter_all_button_label']); ?></span>
                                    </button>
                                <?php } ?>

                                <?php if (!empty($settings['category'])) {
                                    foreach ($category_list_value as $category) {
                                        $categoryName = get_term_by('slug', $category, 'rainbow_projects_category');
                                ?>
                                        <button data-filter=".rbt-<?php echo esc_attr($category); ?> " class="rn-btn"><span class="filter-text"><?php echo esc_html($categoryName->name); ?></span>
                                        </button>
                                        <?php }
                                } else {
                                    ...
                                    if ($terms && !is_wp_error($terms)) {
                                        foreach ($terms as $term) { ?>
                                            <button data-filter=".rbt-<?php echo esc_attr($term->slug); ?>" class="rn-btn"><span class="filter-text"><?php echo esc_html($term->name); ?></span>
                                            </button><?php
                                                    }
                                                }
                                            } ?>
...
<?php }
 ...
 if ($query->have_posts()) { ?>
    <div class="row menu-list row--<?php echo esc_attr($gridspaces); ?> <?php echo esc_attr($filter_class); ?>">
        <?php
        ...
        $terms = get_the_terms(get_the_ID(), 'rainbow_projects_category');
        $termsAssignedCat = '';
        if ($terms && !is_wp_error($terms)) {
            $termsList = [];
        ...
        ?>
            <div class="<?php echo esc_attr($col_class); ?> mt--50 mt_md--30 mt_sm--30 rb-items  <?php echo esc_attr($preview_type_class); ?> <?php echo esc_attr($active); ?> <?php echo esc_attr($termsAssignedCat); ?> <?php echo esc_attr($filter_item_class); ?>">
